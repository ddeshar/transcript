#!/usr/bin/env bash
# One-shot installer for Transcript app from a private GitHub repo over HTTPS (PAT)
# Usage (run as root):
#   sudo REPO_SLUG="ddeshar/transcript" BRANCH="main" bash deploy/install_from_private_github_https.sh
#
# Optional envs:
#   PROJECT_ROOT (/srv/transcript), OPEN_PORTAINER (0/1), OPEN_PGADMIN (0/1)
#   GITHUB_PAT (if set, won’t prompt)
...
echo "[3/7] Preparing directories at $PROJECT_ROOT ..."
mkdir -p "$PROJECT_ROOT"/{media/audio,logs,subtitles,models,deploy}
chown -R "$USER_NAME":"$USER_NAME" "$PROJECT_ROOT"

echo "[4/7] Cloning private repository (HTTPS + PAT)..."
if [[ -z "${GITHUB_PAT:-}" ]]; then
  read -s -p "Enter GitHub Personal Access Token (read access): " GITHUB_PAT
  echo
fi

if [[ ! -d "$PROJECT_ROOT/.git" ]]; then
  echo "Cloning https://github.com/${REPO_SLUG}.git (branch: $BRANCH)"
  # Use a token in URL (simple). Consider using a deploy key + SSH for higher security.
  sudo -u "$USER_NAME" git clone --branch "$BRANCH" --depth 1 \
    "https://${GITHUB_PAT}@github.com/${REPO_SLUG}.git" "$PROJECT_ROOT"
else
...
echo "✅ Done. Visit the app on port 8000. For logs:"
